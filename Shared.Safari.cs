...
namespace Shared
{
    class HelpersSafari
    {
        public static void GenerateUnDownloadFileListFile(string path, string sourceListFile)
        {
            ...
            if (undownloadList.Any())
            {
                var tf = sourceListFile.ChangeFileName("un" + sourceListFile.GetFileNameWithoutExtension());
                tf.WriteAllText(string.Join(Environment.NewLine, undownloadList));

                Process.Start(new ProcessStartInfo
                {
                    FileName = "aria2c",
                    WorkingDirectory = tf.GetDirectoryName(),
                    Arguments = $"--input-file=\"{tf}\" --load-cookies=\"C:\\Users\\Administrator\\Desktop\\Safari\\cookie.txt\"",
                });
            }
